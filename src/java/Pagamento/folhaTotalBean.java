 ...
 * @author amsgama
 */
public class folhaTotalBean implements Serializable {
    ...
    public folhaTotalBean(String xc) {
    }
    ...
    public void consultar() {
        ...
        if (validarHora(hora)) {
            ...
            folhaList = banco.consultaDadosPontoTotal(departamentoSelecionado, funcionarioList_, data, horaData, filtroNomePesquisa, incluirSubSetores);
            //listaPresencaList = getPresenca(listaPresencaList);

            List<folha> folhaAuxList = new ArrayList<folha>();

            totalFuncionarios = 0;
            totalPresentes = 0;
            totalAusentes = 0;

            totalFuncionarios = folhaList.size();

            for (Iterator<folha> it = folhaList.iterator(); it.hasNext();) {
                folha listaPresenca = it.next();
                ...
                if (filtroSelecionado == 3) {
                    if (listaPresenca.getUltimoRegistro().after(data) && listaPresenca.getUltimoRegistro().before(horaData)) {
                        ...
                        folhaAuxList.add(listaPresenca);
                        totalPresentes++;
                    } else {
            ...
            folhaList.clear();
            folhaList = folhaAuxList;

            if (folhaList.isEmpty()) {
    ...
    public List<folha> getPresenca(List<folha> listaPresencaList) {

        List<folha> folhaSaida = new ArrayList<folha>();

        for (Iterator<folha> it = listaPresencaList.iterator(); it.hasNext();) {
    ...
    public void setListaPresencaList(List<folha> folhaList) {
    ...
    public static void main(String[] x) {
        folhaTotalBean d = new folhaTotalBean("");
        d.setData(new Date());
        d.setHora("08:00");
