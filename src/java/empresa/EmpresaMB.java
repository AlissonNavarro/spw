...
public class EmpresaMB implements Serializable {
    ...
    public Empresa consultarEmpresaPorId(int id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                e.setAddress(rs.getString("local"));
    ...
    public boolean updateEmpresa(Empresa e) {
        ...
        try {
            if (con.prepareStatement(query)) {
                con.pstmt.setString(1, e.getCnpj());
                con.pstmt.setString(2, e.getRazaoSocial());
                con.pstmt.setString(3, e.getAddress());
                con.pstmt.setInt(4, e.getCei());
                con.pstmt.setInt(5, e.getId());
                con.executeUpdate();
                return true;
            }
        } catch (SQLException ex) {
